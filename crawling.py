...
chrome_options.add_argument('--headless')  # 크롬 창이 표시되지 않도록 설정

# Chrome 웹 드라이버 실행
driver = webdriver.Chrome('./chromedriver_mac_arm64/chromedriver', options=chrome_options)
driver.get(url)
...
for i in range(1, 1855):  # 1번 페이지부터 1279번 페이지까지 크롤링 수행
    try:
        current_page_num = int(driver.find_element('css selector', '.pagination .active a').text)
        # 다음 페이지로 이동하기
...
# csv 파일로 저장
df.to_csv('output-v3.csv', encoding='utf-8-sig')
