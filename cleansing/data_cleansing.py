...
def preprocess_data_classification():
    ...
    df['피해규모'] = df.apply(calc_damage_scale, axis=1)
    df['피해규모'] = df['피해규모'] / df['공종별 위험도 평가지수']
    df['피해규모'] = df['피해규모'].apply(transform_damage_scale)
...
def preprocess_data_remove_feature():
    ...
    allColumns = df.columns
    params = ['발생일시', '기상상태', '사망자수(명)', '부상자수(명)', '공사비', '공사기간', '공정률', '작업자수', '공종']
    ...
    ct = ColumnTransformer([
    ('onehot', OneHotEncoder(sparse=False), ['날씨', '공종'])], remainder='passthrough'
    )
    ct.fit(df)
    X = ct.transform(df)
    # 컬럼 이름 리스트 생성
    ...
    num_cols = df.columns.tolist()
    ohe = ct.named_transformers_['onehot']
    ohe_cols = ohe.get_feature_names_out(['날씨', '공종']).tolist()
    new_cols = ohe_cols + num_cols
    new_cols.remove('날씨')
    new_cols.remove('공종')
    # DataFrame으로 변환
