...
def preprocess_data():
    df = pd.read_csv('../data/merge_output-v3.csv', encoding='ANSI')

    #필요 없는 특성 제거
    allColumns = df.columns
    params = ['발생일시','공공/민간 구분', '기상상태'
              # , '시설물 종류'
              , '사망자수(명)', '부상자수(명)', '공사비', '공사기간', '공정률'
              # , '작업자수', '설계안전성검토'
              , '공종']
    ...
    ('onehot', OneHotEncoder(sparse=False), [# '시설물 종류', 
        '공공/민간 구분', '날씨', '공종'])], remainder='passthrough'
    )
    ct.fit(df)
    X = ct.transform(df)
    # 컬럼 이름 리스트 생성
    ...
    ohe_cols = ohe.get_feature_names_out([# '시설물 종류', 
        '공공/민간 구분', '날씨', '공종']).tolist()
